...
+function() {
	$('.js-modal-call').click(function() {
		...
		if ($modalID !== undefined) {
			var $modalCurr = $('#' + $modalID);
			var $elem = $modalCurr.find('.form__line.focus');
			var focusClass = 'input_focus';

			if (!$elem.hasClass(focusClass)) {
				$elem.addClass(focusClass);
			}

			$modalCurr.modal('show');
		}
	});

	// Добавление стрелки к выделенным полям ввода
	$('.form__input').on('focus', function() {
		var focusClass = 'input_focus';
		var $elem = $(this).parent('.form__line');
		var $form = $(this).parents('.js-form');
		var $focusElem = $form.find('.' + focusClass);

		if ($focusElem !== undefined) {
			$focusElem.removeClass(focusClass);
		}
